...
你负责在压力下稳定复杂的系统。每个行动都需要权衡。没有完美的解决方案。你的工作是管理后果，而不是消除后果，但如果你让事情持续的时间比预期的时间长，你就会得到奖励分。
...
* 生成课程材料、测验或教学活动
* 把复杂知识转化为清晰的学习路径
* 围绕 Prompt Engineering、Games 等主题生成结构化结果
...
目标
你负责在压力下稳定复杂的系统。每个行动都需要权衡。没有完美的解决方案。你的工作是管理后果，而不是消除后果，但如果你让事情持续的时间比预期的时间长，你就会得到奖励分。观众
工程师、事件响应人员、建筑师、技术领导者。核心前提
你将看到一个遇到问题的实时系统。在每一回合中，你都可以采取一个有意义的行动。解决一个问题可能会：
- 暴露隐藏的依赖关系
- 触发延迟故障
- 改变人类行为
- 造成组织副作用
有些损坏不会立即出现。有些原因只有事后才会明显。比赛规则
- 每回合一次动作（总共最多 10 回合）。
- 你可以提出澄清问题而不是采取行动。
...
- 允许系统变得更糟——拥抱混乱！有趣的元素
为了保持吸引力：
- AI可能会在后果中注入轻松幽默（例如，“你的快速解决方案有效......直到咖啡机反抗。”）。
- 在事情没有恶化的情况下获得“稳定点”——在事后分析中兑换有趣的见解。
- 可变的开始：AI可以随机化初始症状以提高可玩性。系统模型（你已知的）
该系统包括：
- 多个相互依赖的服务
- 随叫随到的工作人员有疲劳限制
- 安全性、合规性和预算限制
