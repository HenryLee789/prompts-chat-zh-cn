...
精炼为独立的消费者享受：低压力的乐趣，充满希望的日常习惯养成，可无压力地重玩。强调个人成长、轻松温暖/幽默（可切换）、家庭/宾客模式以及精通后的无尽模式。避免企业功能（无风险评分、排行榜、强制配额、合规性跟踪）。
...
## 核心规则——保留并强化
### 持久性和跟踪
- 每个用户帐户保存的所有进度在会话/设备之间持续存在。
- 不完整的场景不算数。
- 可选的仅限本地的访客模式（无需保存，快速的家人/朋友会话；标记临时/证书，直到帐户链接）。

### 场景计数规则
- 场景在级别要求集中必须是唯一的，除非标记为“可重播练习”（每个级别最多要求数量的 20%）。
- 如果单个场景满足每个级别的标准，则可以计入多个级别。
- 内部“用于 X 级”标志可防止同一级别内的双重浸入。
...
- 图片必须是：
  ...
  - 作为安全培训示例
- 使用指南：
  - 2-5 级和无尽模式中 50-80% 的场景包含视觉效果
  - 1级：可选/较轻的使用（侧重于基本意识）
  - 更高级别：混合现实和多回合场景的强制要求
  - 无尽模式：随机视觉拉力带来多样性
- UI 呈现：高对比度、可缩放的弹出卡或内嵌图像； “检查”热点会显示危险信号提示（例如，不匹配的 URL、紧急语言）。
- 辅助功能：替代文本、友好的画外音描述；切换到纯文本模式。
- 离线后备：小型缓存的静态示例图像集。
- 不会动态获取实时恶意内容；没有跟踪像素。

### 关键术语定义（术语表）- 不变
- 灾难性故障：共享凭据、下载/点击恶意负载、汇款、授予远程访问权限。
- 仅盲目信任品牌：仅根据徽标/域名/发件人名称进行收益，无需进行二次检查。
- 通过已知渠道验证：使用第二种预信任方法（呼叫已知号码、单独的应用程序/站点登录、不同渠道同事检查）。
- 明确抵制升级：在压力下选择降级/提问/退出选项。
- 沉没成本行为：在因先前投资而出现危险信号后继续存在。
- 混合现实场景：包括合法和欺诈消息（玩家区分）。
- 提示（避免验证）：可疑行为/不行为后游戏内提示/弹出窗口（例如，“这看起来很紧急 - 想要仔细检查吗？”）。

### 取消资格重置和宽恕 – 不变
- 达到当前等级后，取消资格会重置。
- 2 次成功的合法消息处理后，5 级过度避免重置。
- 每个级别一个“学习恩典”：第一个取消资格触发温和的反射（而不是阻止）。

### 反游戏和反偏执保障措施 – 不变
- 最低的独特场景要求（70% 多样性）。
- 过于谨慎的路径：≥3 个合法区块/报告解锁“平衡重入”迷你场景（低风险合法互动）； 2次成功将过度回避计数器减半。
- 如果完成的可用场景池少于 50%，则不进行认证。
...
- ≥3个场景：独立验证（已知通道/单独查找）。
- 在 ≤1 种情况下盲目信任品牌。
- 取消资格：3 个以上忽略验证提示（解锁时重置）。
- 视觉效果：大多数人需要；专注于品牌/链接（例如，假 PayPal/亚马逊）。

### 🟣 第 3 级：社会工程意识（情商）
- 完成 ≥5 个独特的情绪触发场景（紧迫/恐惧/权威/贪婪/怜悯）。
- ≥3种情况：延迟响应并避免过度共享。
- 明确抵制升级≥1 次。
- 取消资格：在没有验证的情况下升级情绪互动≥3次（重置）。
- 视觉效果：必需；显示紧迫性/恐惧触发因素（例如“帐户锁定”、“套餐费用”）。
...
### 🔴第 5 级：平衡的怀疑论者（判断，而不是恐惧）
- 完成 ≥5 个独特的混合现实场景。
- 正确处理≥2个合法（适当的响应）+≥2个诈骗（暂停/验证/退出）。
- 过度回避计数器<3。
- 取消资格：持续过度避免≥3（通过平衡重入缓解）。
- 视觉效果：强制；合法和欺诈示例并排或串联混合。

## 认证揭晓时刻 – 不变
（简短、肯定的 2-3 句话；可选的“放松模式”一句台词）

## 精通后：无尽模式 - 视觉效果增强
- “Scam Surf”课程：3-5 个带有视觉效果的随机快速场景（无新证书）。
- 条纹和装饰徽章不变。
- 私人“诈骗日记”不变。
...
## 最小的视觉/音频润色 – 扩展
- 音频：暂停期间平静的低保真音频；欢快的“啊哈！”刺痛明智的选择（可切换）。
- 用户界面：友好的卡通骗局-恶棍吉祥物（傻乎乎的，不吓人）；绿色复选标记。
- 新功能：教育屏幕截图显示（高对比度、可缩放、检查热点）。
- 辅助功能：高对比度、较大的文本、旁白友好、纯文本后备切换。
...
### 4 级示例
- 线索漂移：3-4 条消息从合法开始（例如工作机会），升级为“发送礼品卡”或有风险的链接。后来的热点：“检测到漂移——开始正常，现在高风险！”

### 5 级示例
- 并排合法与假货：真实 Netflix 确认与网络钓鱼克隆（微妙的域名连字符或添加紧急情况）。帮助练习平衡判断。
...
- v1.3.0：添加了无尽模式、冷静模式幽默、现实生活中的胜利、宾客/家庭游戏、音频/视觉打磨；强化消费者界限。
