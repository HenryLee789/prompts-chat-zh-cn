---
id: "cmk5h32820001kz04x2qq07ad"
slug: "surrealist-painting-description-a-study-of-ren-magrittes-style"
source: "https://github.com/f/prompts.chat"
source_url: "https://prompts.chat/prompts/surrealist-painting-description-a-study-of-ren-magrittes-style"
category: "image-generation"
category_name: "Image Generation"
category_zh: "图像生成"
type: "IMAGE"
translation_status: "machine_translated"
translation_provider: "google"
source_hash: "627297af04ec457506df7c4833d1217ce9deea6c72adf4497ed8e5355653c6ab"
upstream_updated_at: "2026-01-08T22:03:14.090Z"
---
# 超现实主义绘画描述：雷内·马格里特风格研究

> 来源：[prompts.chat](https://github.com/f/prompts.chat)  
> 上游页面：[surrealist-painting-description-a-study-of-ren-magrittes-style](https://prompts.chat/prompts/surrealist-painting-description-a-study-of-ren-magrittes-style)  
> 分类：图像生成（Image Generation / `image-generation`）  
> 类型：`IMAGE`  
> 翻译状态：`machine_translated`

## 中文说明

通过对绘画的详细描述探索雷内·马格里特的超现实主义风格。分析色彩、构图、情绪和叙事元素的使用，以了解马格里特作品的神秘和梦幻般的品质。

## 使用场景

- 用于图像生成相关任务的 AI prompt 输入。
- 用于图像生成模型的画面描述、风格控制和视觉创意生成。
- 适合围绕 Art、Creative Writing、Descriptions、History 等主题快速生成可复用结果。

## 适用人群

- 设计师
- AI 绘图用户
- 内容创作者

## 中文 Prompt 正文

```md
{
  “颜色”：{
    "color_temper": "暖色",
    "contrast_level": "高",
    “主导调色板”：[
      “红色”，
      “橙色”，
      “灰蓝色”，
      “浅灰色”
    ]
  },
  “组成”：{
    "camera_angle": "眼睛水平",
    "depth_of_field": "深",
    "focus": "红太阳",
    “取景”：“构图横向分层，前景为石墙，中景为一排树木，背景为天空。红日位于中央，形成强烈的焦点。”
  },
  "description_short": "雷内·马格里特的超现实主义画作，描绘了一个充满活力的红色太阳或球体悬挂在一片柔和的灰色树林前，映衬着火红色和橙色的天空。前景是一面带有瓮的石墙。",
  “环境”：{
    "location_type": "室外",
    "setting_details": "从低矮的石墙后面看，场景似乎是一个公园或一个正式的花园。修剪整齐的草坪将墙与茂密的绿叶树林隔开。",
    "time_of_day": "晚上",
    “天气”：“晴”
  },
  “照明”：{
    “强度”：“强”，
    "source_direction": "未知",
    “类型”：“超现实”
  },
  “心情”：{
    "atmosphere": "神秘而梦幻的寂静",
    "emotional_tone": "超现实"
  },
  “叙述元素”：{
    "environmental_storytelling": "太阳不可能放置在树木前面，这颠覆了现实，创造了一种惊奇感和智力悖论。普通的人造墙与非凡的自然景观形成鲜明对比，质疑观看者对空间和现实的感知。",
    "implied_action": "场景完全静态，捕捉到一个违背天体自然运动的时刻。"
  },
  “对象”：[
    《红日》、
    “树”，
    “石墙”，
    《石瓮》、
    “天空”，
    “草坪”
  ],
  “人”：{
    “计数”：“0”
  },
  "prompt": "这是一幅雷内·马格利特风格的高度详细的超现实主义油画。一个巨大的、完美的圆形、充满活力的红色太阳悬浮在半空中，不可思议地位于一片茂密的灰蓝色树木前面。后面的天空发出强烈的渐变光，从顶部的火红色到地平线的温暖橙色。前景是精心渲染的浅灰色石墙，底座框架上有一个古典的瓮。画面底部整体气氛神秘、静谧、梦幻，暖色与冷色形成鲜明对比。",
  “风格”：{
    "art_style": "超现实主义",
    “影响”：[
      《雷内·马格里特》
    ],
    “媒介”：“绘画”
  },
  “技术标签”：[
    “超现实主义”，
    “油画”，
    “风景”，
    “并列”，
    “象征主义”，
    “高对比度”，
    “鲜艳的色彩”
  ],
  "use_case": "艺术史数据集，风格迁移模型训练，AI艺术提示超现实主义灵感。",
  “uuid”：“b6ec5553-4157-4c02-8a86-6de9c2084f67”
}
```

---

## English Original

### Title

Surrealist Painting Description: A Study of René Magritte's Style

### Description

Explore the surrealist style of René Magritte through a detailed description of a painting. Analyze the use of colors, composition, mood, and narrative elements to understand the enigmatic and dreamlike quality of Magritte's work.

### Prompt

```md
{
  "colors": {
    "color_temperature": "warm",
    "contrast_level": "high",
    "dominant_palette": [
      "red",
      "orange",
      "grey-blue",
      "light grey"
    ]
  },
  "composition": {
    "camera_angle": "eye-level",
    "depth_of_field": "deep",
    "focus": "Red sun",
    "framing": "The composition is horizontally layered, with a stone wall in the foreground, a line of trees in the midground, and the sky in the background. The red sun is centrally located, creating a strong focal point."
  },
  "description_short": "A surrealist painting by René Magritte depicting a vibrant red sun or orb hanging in front of a forest of muted grey trees, set against a fiery red and orange sky. A stone wall with an urn stands in the foreground.",
  "environment": {
    "location_type": "outdoor",
    "setting_details": "The scene appears to be a park or a formal garden, viewed from behind a low stone wall. A manicured lawn separates the wall from a dense grove of leafy trees.",
    "time_of_day": "evening",
    "weather": "clear"
  },
  "lighting": {
    "intensity": "strong",
    "source_direction": "unknown",
    "type": "surreal"
  },
  "mood": {
    "atmosphere": "Enigmatic and dreamlike stillness",
    "emotional_tone": "surreal"
  },
  "narrative_elements": {
    "environmental_storytelling": "The impossible placement of the sun in front of the trees subverts reality, creating a sense of wonder and intellectual paradox. The ordinary, man-made wall contrasts with the extraordinary natural scene, questioning the viewer's perception of space and reality.",
    "implied_action": "The scene is completely static, capturing a moment that defies the natural movement of celestial bodies."
  },
  "objects": [
    "Red sun",
    "Trees",
    "Stone wall",
    "Stone urn",
    "Sky",
    "Lawn"
  ],
  "people": {
    "count": "0"
  },
  "prompt": "A highly detailed surrealist oil painting in the style of René Magritte. A large, perfectly circular, vibrant red sun is suspended in mid-air, impossibly positioned in front of a dense forest of muted, grey-blue trees. The sky behind glows with an intense gradient, from fiery red at the top to a warm orange at the horizon. In the foreground, a meticulously rendered light-grey stone wall with a classical urn on a pedestal frames the bottom of the scene. The overall mood is mysterious, silent, and dreamlike, with a stark contrast between warm and cool colors.",
  "style": {
    "art_style": "surrealism",
    "influences": [
      "René Magritte"
    ],
    "medium": "painting"
  },
  "technical_tags": [
    "surrealism",
    "oil painting",
    "landscape",
    "juxtaposition",
    "symbolism",
    "high contrast",
    "vibrant colors"
  ],
  "use_case": "Art history dataset, style transfer model training, AI art prompt inspiration for surrealism.",
  "uuid": "b6ec5553-4157-4c02-8a86-6de9c2084f67"
}
```

### Metadata

| Field | Value |
| --- | --- |
| Source | [prompts.chat](https://github.com/f/prompts.chat) |
| Upstream URL | [surrealist-painting-description-a-study-of-ren-magrittes-style](https://prompts.chat/prompts/surrealist-painting-description-a-study-of-ren-magrittes-style) |
| Category | Image Generation (`image-generation`) |
| Type | `IMAGE` |
| Tags | Art, Creative Writing, Descriptions, History |
| Contributors | gunebak4n |
| Updated At | 2026-01-08T22:03:14.090Z |
