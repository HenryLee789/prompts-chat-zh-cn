...
[城市名称] 的欢乐微型世界完全由[地标名称] 周围的彩色粘土模型建造而成。可见的指纹、有机的曲线和充满活力的俏皮色彩营造出童趣十足的手工外观。城市名称出现在顶部，场景中没有其他文字或标牌。
...
```md
生成一个异想天开的微型世界，其中 ${landmark_name} 完全由彩色粘土制作而成。每个元素（建筑物、树木、水道和城市特征）都应该是手工雕刻的，具有明显的指纹和有机粘土纹理。使用俏皮、童趣的风格和鲜艳的色彩：明亮的蔚蓝天空、蓬松的奶油色云彩、翠绿的树木以及暖黄色、橙色、红色和蓝色的建筑物。每个表面和柔和的曲线都应该体现出手工制作的品质。从广阔的视角捕捉，以和谐、欢乐的构图展示整个微缩景观。在顶部中心，以干净、粗体、友好的圆形字体添加城市名称 ${city_name}，与俏皮的粘土美学相匹配。文本应该清晰易读，与天空形成高对比度，具有微妙的深度，就好像它也是由粘土制成的（轻微的 3D 粘土字体），但保持简单，不要过于详细。场景中其他任何地方均不得包含其他文字、文字或标牌。只有雕塑粘土元素才能通过可识别的建筑特征来定义位置。 1080x1080 尺寸。
```
...
A joyful miniature world of [CITY NAME] built entirely from colorful modeling clay around [LANDMARK NAME]. Visible fingerprints, organic curves, and vibrant playful colors create a childlike handmade look. The city name appears at the top, with no other text or signage anywhere in the scene.
