---
id: "cmlz60nmp0001k204h2n5qk5p"
slug: "vintage-black-and-white-photograph-of-galata-tower"
source: "https://github.com/f/prompts.chat"
source_url: "https://prompts.chat/prompts/vintage-black-and-white-photograph-of-galata-tower"
category: "image-generation"
category_name: "Image Generation"
category_zh: "图像生成"
type: "IMAGE"
translation_status: "machine_translated"
translation_provider: "google"
source_hash: "eb84afff41f30873a2a3ddcd173ae03d01d081711535950aa337e781c8c5897d"
upstream_updated_at: "2026-02-23T12:41:13.784Z"
---
# 加拉达塔的老式黑白照片

> 来源：[prompts.chat](https://github.com/f/prompts.chat)  
> 上游页面：[vintage-black-and-white-photograph-of-galata-tower](https://prompts.chat/prompts/vintage-black-and-white-photograph-of-galata-tower)  
> 分类：图像生成（Image Generation / `image-generation`）  
> 类型：`IMAGE`  
> 翻译状态：`machine_translated`

## 中文说明

本提示描述了伊斯坦布尔加拉达塔的一张老式黑白照片，周围环绕着高大的柏树，背景是历史悠久的奥斯曼时代房屋和墓地。非常适合历史和建筑研究、复古照片修复和文化遗产记录。

## 使用场景

- 用于图像生成相关任务的 AI prompt 输入。
- 用于图像生成模型的画面描述、风格控制和视觉创意生成。

## 适用人群

- 设计师
- AI 绘图用户
- 内容创作者

## 中文 Prompt 正文

```md
{
  “颜色”：{
    "color_temper": "中性",
    "contrast_level": "高",
    “主导调色板”：[
      “黑”，
      “白色”，
      “灰色”
    ]
  },
  “组成”：{
    "camera_angle": "广角镜头",
    "depth_of_field": "深",
    “焦点”：“加拉达塔”，
    “framing”：“加拉达塔位于图像上半部分的中央，两侧由高大、深色的柏树垂直框架。”
  },
  "description_short": "一张伊斯坦布尔加拉达塔的老式黑白照片，从墓地看去，那里有古老的木屋，周围环绕着高大的柏树。",
  “环境”：{
    "location_type": "城市景观",
    "setting_details": "背景是伊斯坦布尔的一个历史街区，很可能是加拉塔。背景是标志性的加拉塔石塔。中间是古老的、可能是木制的、奥斯曼时代的建筑。前景是一片杂乱的区域，似乎是一个墓地，有风化的墓碑或从地面突出的柱子。",
    "time_of_day": "下午",
    “天气”：“晴”
  },
  “照明”：{
    “强度”：“强”，
    "source_direction": "侧面",
    “类型”：“自然”
  },
  “心情”：{
    "atmosphere": "永恒而怀旧的一瞥过去。",
    "emotional_tone": "忧郁"
  },
  “叙述元素”：{
    "character_interactions": "在中景中可以看到两个人物，站在建筑物附近。他们的互动很少，作为场景日常生活的一部分而不是焦点出现。",
    "environmental_storytelling": "该图像将塔楼经久不衰的石碑与腐烂的木结构和墓地并置，暗示了历史、记忆和时间流逝的主题。",
    "implied_action": "场景静态而安静，捕捉历史名城的宁静时刻。"
  },
  “对象”：[
    “加拉达塔”，
    “柏树”，
    “木屋”，
    “墓碑”，
    “石墙”
  ],
  “人”：{
    “年龄”：[
      “成人”
    ],
    "clothing_style": "奥斯曼帝国时代的传统服装",
    “计数”：“2”，
    “性别”：[
      “男”
    ]
  },
  "prompt": "一张古老的、高对比度的黑白照片，拍摄的是伊斯坦布尔历史悠久的加拉达塔。标志性的石塔，其圆锥形屋顶在明亮的天空的映衬下在背景中拔地而起。场景被高大、黑暗、气势磅礴的柏树所包围。前景和中景，一座古老的墓地，有风化的墓碑和破旧的奥斯曼木屋，营造出一种历史感和忧郁感。灯光是明亮的自然阳光，投射出锐利的光线。阴影是永恒和怀旧的。”,
  “风格”：{
    "art_style": "现实",
    “影响”：[
      “19世纪摄影”，
      “旅游摄影”，
      “纪录片”
    ],
    “中”：“摄影”
  },
  “技术标签”：[
    “黑与白”，
    “单色”，
    “复古照片”，
    “历史”，
    “高对比度”，
    “胶片颗粒”，
    “加拉达塔”，
    “伊斯坦布尔”，
    “奥斯曼建筑”，
    “垂直构图”
  ],
  "use_case": "历史和建筑研究、复古照片修复数据集、文化遗产文献。",
  “uuid”：“4b0a2894-4d0f-4bd1-82ee-5ee7cf81e135”
}
```

---

## English Original

### Title

Vintage Black and White Photograph of Galata Tower

### Description

This prompt describes a vintage black and white photograph of the Galata Tower in Istanbul, framed by tall cypress trees, against a backdrop of historic Ottoman-era houses and a cemetery. Ideal for historical and architectural studies, vintage photo restoration, and cultural heritage documentation.

### Prompt

```md
{
  "colors": {
    "color_temperature": "neutral",
    "contrast_level": "high",
    "dominant_palette": [
      "black",
      "white",
      "grey"
    ]
  },
  "composition": {
    "camera_angle": "wide shot",
    "depth_of_field": "deep",
    "focus": "Galata Tower",
    "framing": "The Galata Tower is centrally placed in the upper half of the image, framed vertically by tall, dark cypress trees on both sides."
  },
  "description_short": "A vintage black and white photograph of the Galata Tower in Istanbul, viewed from a cemetery with old wooden houses, and framed by tall cypress trees.",
  "environment": {
    "location_type": "cityscape",
    "setting_details": "The setting is a historic neighborhood in Istanbul, likely Galata. In the background stands the iconic stone Galata Tower. The middle ground features old, possibly wooden, Ottoman-era buildings. The foreground is an unkempt area, appearing to be a cemetery with weathered grave markers or posts protruding from the earth.",
    "time_of_day": "afternoon",
    "weather": "clear"
  },
  "lighting": {
    "intensity": "strong",
    "source_direction": "side",
    "type": "natural"
  },
  "mood": {
    "atmosphere": "A timeless and nostalgic glimpse into the past.",
    "emotional_tone": "melancholic"
  },
  "narrative_elements": {
    "character_interactions": "Two figures are visible in the mid-ground, standing near a building. Their interaction is minimal, appearing as part of the daily life of the scene rather than a focal point.",
    "environmental_storytelling": "The image juxtaposes the enduring stone monument of the tower with the decaying wooden structures and the cemetery, suggesting themes of history, memory, and the passage of time.",
    "implied_action": "The scene is static and quiet, capturing a moment of stillness in a historic city."
  },
  "objects": [
    "Galata Tower",
    "Cypress trees",
    "Wooden houses",
    "Tombstones",
    "Stone walls"
  ],
  "people": {
    "ages": [
      "adult"
    ],
    "clothing_style": "traditional Ottoman-era attire",
    "count": "2",
    "genders": [
      "male"
    ]
  },
  "prompt": "A vintage, high-contrast black and white photograph of the historic Galata Tower in Istanbul. The iconic stone tower with its conical roof rises in the background against a bright sky. The scene is framed by tall, dark, imposing cypress trees. In the foreground and middle ground, an old cemetery with weathered tombstones and dilapidated wooden Ottoman houses creates a sense of history and melancholy. The lighting is bright natural sunlight, casting sharp shadows. The mood is timeless and nostalgic.",
  "style": {
    "art_style": "realistic",
    "influences": [
      "19th-century photography",
      "travel photography",
      "documentary"
    ],
    "medium": "photography"
  },
  "technical_tags": [
    "black and white",
    "monochrome",
    "vintage photograph",
    "historical",
    "high contrast",
    "film grain",
    "Galata Tower",
    "Istanbul",
    "Ottoman architecture",
    "vertical composition"
  ],
  "use_case": "Historical and architectural studies, dataset for vintage photo restoration, cultural heritage documentation.",
  "uuid": "4b0a2894-4d0f-4bd1-82ee-5ee7cf81e135"
}

```

### Metadata

| Field | Value |
| --- | --- |
| Source | [prompts.chat](https://github.com/f/prompts.chat) |
| Upstream URL | [vintage-black-and-white-photograph-of-galata-tower](https://prompts.chat/prompts/vintage-black-and-white-photograph-of-galata-tower) |
| Category | Image Generation (`image-generation`) |
| Type | `IMAGE` |
| Tags | None |
| Contributors | gunebak4n |
| Updated At | 2026-02-23T12:41:13.784Z |
