# 使用 Remotion 的 Tips.chat 宣传视频

## 中文说明

此提示指导使用Remotion为prompts.chat创建30秒的宣传视频。它概述了有效展示平台功能和社区所需的资源、颜色主题、字体样式和场景结构。该提示包括动画技术和过渡，以确保流畅、引人入胜的观看体验，强调平台的全球影响力和各种可用的提示类型。
...
```md
为 Prompss.chat 制作 30 秒的宣传视频

所需资产

- https://prompts.chat/logo.svg - 标志 SVG
- https://raw.githubusercontent.com/flekschas/simple-world-map/refs/heads/master/world-map.svg - 全球社区场景的世界地图 SVG
...
- 标志出现
- 徽标居中，随弹簧动画缩放
- 动画后：“prompts.chat”文本在徽标下方从左到右显示
剪辑路径
- 标语出现：“AI提示的免费社交平台”
...
- 全屏世界地图（25% 不透明度）作为背景
- 主要城市（洛杉矶、纽约、多伦多、圣保罗、
伦敦、巴黎、柏林、拉各斯、莫斯科、迪拜、孟买、北京、东京、
新加坡、悉尼、华沙）
- 每个点都有外脉冲环、内脉冲和带有发光的中心点
- 标题：“提示创作者的全球社区”
- 统计行：8k+ 用户、3k+ 每日访客、1k+ 提示、300+ 贡献者、
10+ 种语言
- 底部渐变覆盖以提高文本可读性

场景3：解决方案（2.5秒）

- 三个单词随着弹簧动画依次出现：“发现”。 “分享。”

“收集。”
- 每个单词都有不同的颜色（主色、重音、主光）

场景 4：为所有人打造（4 秒）

- 屏幕边缘周围有 8 个浮动角色图标，带有正弦/余弦波浮动
动画
- 人物角色：学生、教师、研究人员、开发人员、艺术家、作家、
营销人员、企业家
- 每个都有 130x130 图标容器，带有彩色背景/边框
- 中心标题：“为所有人打造”
- 副标题：“距离下一个突破只有一步之遥。”
...
- 标题：“满足各种需求的提示”
- 类似浏览器的框架 (1400x800)，带有 macOS 交通灯和 URL 栏显示
“prompts.chat”
- 砖石骨架屏幕截图垂直滚动，带有缓动动画（立方缓入缓出）
- 边缘周围有 7 个带有图标的浮动药丸形状标签：
  - 文本（紫色）、图像（粉色）、视频（琥珀色）、音频（绿色）、工作流程
（紫色）、技能（青色）、JSON（红色）
...
- 4 张功能卡依次出现并带有弹簧动画：
  - 提示库（书籍图标）-“所有类别的数千个提示”
  - 技能和工作流程（螺栓图标）-“自动化多步骤 AI 任务”
  - 社区（用户图标）-“与创作者分享和发现”
  - 开源（圆圈加号图标） - “具有完全隐私的自我托管”

场景 7：社会证明（4s）

- 动画 GitHub 星计数器 (0 → 143,000+)
- 计数旁边的星形图标
- 徽章：“第一个即时图书馆 - 自 2022 年 12 月以来”，带有奖杯图标
- 文本：“由 OpenAI 联合创始人认可 • 被哈佛大学、哥伦比亚大学等使用”

场景 8：CTA（3.5 秒）

- 背景发光动画（脉冲径向渐变）
- 标题：“从今天开始探索”
- 带徽标的大按钮+“prompts.chat”文本（渐变背景，微妙
脉冲）
- 副标题：“免费和开源”
...
- 所有图标的自定义 SVG 图标组件（无表情符号）
- 带有提示的徽标组件。聊天“P”路径
-FeatureCard可重复使用组件
- 用于场景管理的 TransitionSeries
```
