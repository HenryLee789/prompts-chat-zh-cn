...
总结当前/指定奥运会（例如 2026 年米兰科尔蒂纳奥运会）未来 7 天内即将举行的奥运会赛事（奖牌、仪式）。优先考虑流行的运动（花样滑冰、滑雪、曲棍球）。包括美国广播 (NBC/Peacock) 和当地时间 (EST)。使用每日Markdown表，专注于关键的决赛/奖牌，跳过次要的预赛。
...
* 围绕 Spotlight、Entertainment 等主题生成结构化结果
...
Base the list on the most up-to-date schedules from reliable sources (e.g., Olympics.com official schedule, NBCOlympics.com, TeamUSA.com, ESPN, BBC Sport, Wikipedia Olympic pages, official broadcaster sites).如果存在时间/日期冲突，请优先考虑国际奥委会或主办广播公司的官方公告。以简短的注释部分结束响应，内容包括：
