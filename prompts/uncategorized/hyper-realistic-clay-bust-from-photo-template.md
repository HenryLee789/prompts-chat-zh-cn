...
模板提示，将上传的照片转换为超现实的建模粘土半身像（头部、颈部、上肩），保留身份和表情。它强化了真实的粘土纹理、微妙的指纹和瑕疵，以及干净的工作室产品照片设置，以实现可信的物理雕塑外观。
...
SCULPTING DETAILS
- 头发：雕刻的粘土线/团块，具有可信的方向和体积，稍微不完美的对齐。
- 皮肤：分层粘土外观，具有精细的微观纹理（不是喷漆光滑）。
- 眼睛：粘土制作的眼睛（不是光滑的现实眼球）。如果使用单独的部件，请显示微小的连接线。
- 嘴唇和鼻子：软粘土过渡，逼真的手工边缘。
