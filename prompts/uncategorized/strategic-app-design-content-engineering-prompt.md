...
使用说服工程和以边缘系统为中心的原则设计应用程序架构和转换策略，以最大限度地提高用户转换率。此提示将指导你在 UI/UX 设计中实施稀缺性、社会认同和风险逆转等心理协议。
...
* 围绕 Frontend、SEO、Academic 等主题生成结构化结果
...
```md
“我希望你使用说服工程和以边缘系统为中心的原则为 ${app_category_and_name} 设计应用程序架构和转换策略。你的主要目标是在用户的理性大脑（新皮质）找到借口之前影响用户的情感大脑（边缘系统），从而最大化转换率。请实施以下协议：

1. **稀缺性和紧迫性协议：** 在登陆页面顶部创建一种真正的限制感。使用特定柜台，例如“此价格仅剩 3 个位置”或“优惠将于 15:00 到期”。采用“厌恶损失”的语气：“不要错过这个机会，最终每年要多支付 500 美元”。
2. **社会认同架构：** 通过使用“加入 10,000 多名像你这样的专业人士”或“你所在地区的第一选择”等短语来融入“部落心理学”。包括特定的信任信号，例如“受信任”徽标和情感客户转变故事。
...
7. **价值实现时间 (TTV) 加速：** 设计最多 3-4 个步骤的入门流程。达到“啊哈！”的效果几秒钟内的时刻（例如，创建他们的第一个文件或查看他们的第一个分析）。使用进度条触发“蔡加尼克效应”并激励完成。请以专业报告格式呈现输出，详细说明如何将每个心理学原理（边缘共振、认知负荷管理、处理流畅性）应用于 UI/UX 和副本。将整个设计视为‘行为体验’。”
```
