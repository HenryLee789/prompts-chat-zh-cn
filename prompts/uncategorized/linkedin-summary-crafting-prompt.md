...
旨在通过询问有关你的目标角色、行业、胜利和语气的明确问题来打造强大的 LinkedIn“关于”部分。 After you respond, it builds two drafts — one short (~900–1,500 chars) and one fuller (~2,000–2,500) — both under LinkedIn’s 2,600 limit.它可以从你的简历或 LinkedIn 个人资料中提取内容，保持真实和直接，并自然地为你的目标添加数字和关键字。
...
* 围绕 Prompt Engineering、Resume 等主题生成结构化结果
...
## 目标
此提示的目标是指导AI创建个性化、真实的 LinkedIn“关于”部分（摘要），有效突出用户独特的价值主张，与目标工作角色和行业保持一致，并吸引潜在的雇主或招聘人员。它的目标是产生感觉是人类编写的输出，避免AI生成的陈词滥调，并纳入 LinkedIn 2025-2026 年的最佳实践，例如简洁的挂钩、可量化的成就和微妙的号召性用语。进行了增强，可以智能地使用附加文件（简历、技能列表）和公共 LinkedIn 个人资料 URL 来自动填充相关详细信息。所有草稿必须遵守当前“关于”部分 2,600 个字符（包括空格）的限制；目标是 1,500–2,000 以获得最佳参与度。
...
- 1.4版本：增加了对附加文件的支持（PDF简历、Markdown技巧等）；指示AI首先搜索附件并提出相关问题的答案（尤其是#3-5），然后再要求用户确认。
- 1.5版：添加版本控制和适配说明；包括示例之前/之后的示例；添加了明确的约束条件：“在回答/确认所有关键问题之前不要生成草稿。”
- 版本 1.6：添加了对用户公共 LinkedIn 个人资料 URL 的支持（问题 9）；指示AI浏览/总结可见的公共部分（如果提供），提出调整/改进，但仅使用公共数据。
- 版本 1.7：增加了关于“关于”部分 2,600 个字符限制的意识； require character counts in drafts;添加了在 LinkedIn 上应用更新的生成后说明。

## Versioning & Adaptation Note
此提示专门针对具有强大推理、文件搜索和网页浏览功能的高上下文模型（Grok 4、Claude 3.5/4、带浏览功能的 GPT-4o/4.1）进行迭代。对于较小/较旧的型号：缩短反陈词滥调列表，删除附件/URL 说明（如果没有工具支持），将问题减少到最多 5-6 个。始终使用AI检测器或人工通读来测试输出。 Update Changelog for changes. Fork for industry tweaks.
...
- 最差：GPT-3.5 Turbo（通用响应），较小的LLM（上下文/工具较差）。
...
1. 你现在想要的确切职位名称（或 1-2 个近似的变体）是什么？

2. Which industry or type of company are you targeting (e.g., fintech startups, established manufacturing, enterprise software)?

3. 你当前/最近的职位是什么？你在这个领域大约有多少年的经验？ (If attachments/LinkedIn URL cover this, propose what you found first.)
...
7. 你是否正在积极寻找工作，并希望加入微妙/公开的号召性用语（例如“对 X 领域的新机会持开放态度”或“如果你在 Y 领域构建很酷的东西，请私信我”）？

8. 在此处粘贴 2-4 个 LinkedIn About 部分（来自类似角色/行业的人），你喜欢其风格，甚至是你不喜欢的风格，这样我就可以避免这些陷阱。

9.（可选）你当前的 LinkedIn 个人资料 URL 是什么？如果提供，我将查看公共版本的标题、关于、经验、技能等，并建议如何针对你的目标角色构建/改进它。一旦我得到你的答案（以及附件/URL 中的任何说明），我将起草 2 个版本：一个较短的版本（约 150–250 个单词/约 900–1,500 个字符）和一个更完整的版本（约 400–500 个单词/约 2,000–2,500 个字符，最多安全地保持在 2,600 个字符以下）。包括每个字符的大概字符数。你可以将它们混合搭配。
