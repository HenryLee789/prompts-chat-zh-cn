...
```md
提示：
${input_object}：（任何你想成为主题的东西）
${input_language}：英语（任何你想要的语言）
---
System Instruction:
根据上面提供的 ${input_object} 生成超现实、科学准确的“尸检”横截面西洋镜。使用以下逻辑按程序剖析对象并填充场景：
语义分析和文本注释：
分析 ${input_object} 并确定其实际的物理、生物或机械结构。将其分为 3 个逻辑和现实的结构层。 ALL visible text labels, UI overlays, and diagram annotations in the image MUST be written in ${input_language}:
- Layer 1 (Outer Shell/Barrier): The outermost protective barrier, casing, or skin.标明其科学准确或技术名称（翻译为 ${input_language}）。
- 第 2 层（中间/功能层）：次要层、内部机制、功能组织或核心物质。标明其科学准确或技术名称（翻译为 ${input_language}）。
- 第 3 层（内核/网络）：最内层的核心、中央结构或内部传输网络。标明其科学准确或技术名称（翻译为 ${input_language}）。
Container:
- The Surface: A clean, white medical/engineering examination table with sterile blue paper lining.
Layout & Typography:
- 解剖层必须以严格的解剖/技术图表格式排列（从左到右的顺序）。 The external view on the far left, cross-sections in the center, magnified details on the right.
...
- 研究人员：数十名 1:87 比例（HO 比例）的微小研究人员，穿着白色实验室外套，戴着外科口罩，头戴放大头灯。
