...
创建一个简洁、用户友好的摘要，介绍新电视节目的首播和在指定的下周开始的回归季。输出每天使用单独的Markdown表（以日期为标题），重点关注主要的流媒体服务，同时注明突出的广播服务。这可以帮助用户快速计划他们的观看，而不会因空闲时间或过多的小型节目而造成混乱。添加了下周将上线的电影
...
* 围绕 Entertainment、Prompt Engineering 等主题生成结构化结果
...
列出首播或回归的电视节目（新季开始、从中断/中断中恢复的剧集以及全新剧集首播），以及从今天开始的未来 7 天内发布到流媒体服务的新电影。使用单独的Markdown表来组织每天至少有一次值得注意的首映/回归/发布的信息。将日期作为 3 级标题放置在每个表格上方（例如，### 2026 年 2 月 6 日）。跳过没有重大活动的日子——不要提及空虚的日子。在每个表中使用这些确切的列：
- 姓名
- 类型（“电视节目”或“电影”）
