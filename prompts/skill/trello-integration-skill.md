...
此技能允许你与 Trello 帐户交互以自动列出面板、查看列表和创建卡片。
...
* 定义输入、流程、约束和输出格式
* 帮助智能体稳定执行专业任务
* 围绕 Agent、Automation、Project Management 等主题生成结构化结果
...
---
名称： trello 集成技能
描述：此技能允许你与 Trello 帐户交互以自动列出面板、查看列表和创建卡片。
---
...
要在本地使用此技能，你需要提供 Trello Developer API 凭据。

1. 在 [Trello 开发者门户（Power-Ups 管理员）](https://trello.com/app-key) 生成你的凭据。
2. 创建 API 密钥。
3. 生成秘密令牌（读/写访问）。
...
3. **返回报告**：与用户确认创建成功，并提供新创建的 Trello 卡的直接 URL。文件：create_card.js
const 路径 = require('路径');
...
if (!API_KEY || !TOKEN) {
    console.error("bug：.env 文件中缺少 TRELLO_API_KEY 或 TRELLO_TOKEN。");
    进程.退出(1);
}
...
if (!listId || !cardName) {
    控制台.bug(`Usage: node create_card.js <listId> "${card_name}" ["${card_description}"]`);
    进程.退出(1);
}
...
如果（！response.ok）{
            ...
            抛出新bug（`HTTP error! status: ${response.status}, message: ${errText}`）；
        }
...
异步函数 listBoards() {
    ...
    尝试{
        ...
        if (!response.ok) 抛出新bug(`HTTP error! status: ${response.status}`);
