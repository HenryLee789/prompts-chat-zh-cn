...
```md
建立一个名为“零到一”的独立创始人发布系统——一个为期 14 天的结构化系统，用于从创意到第一个付费客户。核心特点：
- 想法摄入：用户输入他们的想法、目标客户和预期价格点。 [LLM API] 通过提出 3 个澄清问题来验证输入 — 在生成任何模板之前强制明确性
- 个性化剧本：14 天日历，每天都有特定任务、定制模板和成功指标。所有模板均由 [LLM API] 使用用户的特定想法和客户生成，而不是通用的。第一天：问题验证脚本。第三天：着陆页文案。第五天：外展邮件。第七天：客户访谈指导。第 10 天：销售对话框架。第 14 天：事后分析模板
