...
函数 parseTrendingProjects(html: string, limit: number): TrendingProject[] {
  const 项目：TrendingProject[] = []；
  尝试{
    const ArticleRegex = /<article[^>]*>([\s\S]*?)<\/article>/g;
    const 文章 = html.match(articleRegex) || []；
    constarticlesToProcess =articles.slice(0, 限制);
    articlesToProcess.forEach((文章, 索引) => {
      尝试{
        const headerMatch = Article.match(/<h[12][^>]*>([\s\S]*?)<\/h[12]>/);
        让 repoName: string |空=空；
        如果（标题匹配）{
          常量标题内容=标题匹配[1]；
          const validLinkMatch = headerContent.match(
            /<a[^>]*href="\/([^\/"\/]+\/[^\/"\/]+)"[^>]*>(?![^<]*登录)/
          ）；
          如果（有效链接匹配）{
        ...
        if (!repoName) {
          const repoMatch = 文章.match(
            /<a[^>]*href="\/([a-zA-Z0-9_.-]+\/[a-zA-Z0-9_.-]+)"[^>]*>(?!.*(?:登录|观星者|分叉|问题))/
          ）；
          存储库名称 = 存储库匹配 ? repoMatch[1]：空；
        }
        ...
        const 语言 = langMatch ？ langMatch[1].trim() : "未知";
        const starMatch = Article.match(/<a[^>]*href="\/[^"]+\/stargazers"[^>]*>(\d[\d,]*)\s*stars?/);
        常量总星星=星星匹配？星星匹配[1]：“0”；
        const starAddedMatch = Article.match(/(\d[\d,]*)\s*stars?\s*(?:今天|本周)/i);
...
函数 formatProjects(项目: TrendingProject[], 选项: TrendingOptions): string {
  if (projects.length === 0) {
    return "# GitHub Trending - No Projects Found\n\n没有找到trending项目，可能是网络问题或页面结构变化。";
  }
  const periodLabel = options.period === "每日" ? “每日” : “每周”;
  const languageLabel = 选项. 语言 ? `Language: ${options.language}` : "语言：全部";
  const Today = new Date().toISOString().split("T")[0];
  让输出= `# GitHub Trending Projects - ${periodLabel} (${today})\n\n`；
  输出+=`📊 **Total:** ${projects.length} projects | **${languageLabel}** | **Period:** ${periodLabel}\n\n`；
  输出+=`---\n\n`；
  项目.forEach((项目) => {
    输出+=`## ${project.rank}. ${project.name} - ⭐ ${project.stars}`；
    如果（project.starsThisPeriod）{
      输出+=` (${project.starsThisPeriod} this ${options.period})`；
    }
    输出+=`\n`；
    输出+=`**Language:** ${project.language}\n`；
    输出+=`**Description:** ${project.description}\n`；
    输出+=`**URL:** ${project.url}\n\n`；
  });
  输出+=`---\n`；
  输出+=`📊 Data from: https://github.com/trending\n`；
  返回输出；
}
...
异步函数 main() {
  ...
  尝试{
    ...
    const formatted = formatProjects(项目、选项);
    控制台.log（格式化）；
    if (projects.length === 0) {
...
此工作流使用 GetTrending.ts 工具从 GitHub 获取当前最热门的项目列表，支持按时间周期（每日/每周）和编程语言筛选。

## 何时使用

当用户请求以下任何内容时使用此工作流程：
- “显示 github 趋势”/“github 趋势”
- “显示热门项目” / “看看有什么热门项目”
- “每周趋势”/“本周热门项目”
- “每日趋势” / “今日热门项目”
- “TypeScript 趋势” / “Python 趋势” / 按语言筛选
- “github上什么最火”/“github上什么最火”
...
- **编程语言**：可选（如TypeScript、Python、Go、Rust等）
- **项目数量**：默认10个
...
- 数据基础更新一次
- 获取GitHub API令牌
- 使用公开的GitHub trendingPages
- 支持的语言参数不区分大小写
文件：工具/GenerateDashboard.ts
#!/usr/bin/env 面包
/**
 * GitHub 趋势仪表板生成器
 *
 ...
 * 示例：
 * ./生成仪表板.ts
 * ./GenerateDashboard.ts --period daily --语言 TypeScript --include-news
 * ./GenerateDashboard.ts --limit 20 --output ~/trends.html
 */

从 'handlebars' 导入 Handlebars；
从 './Lib/types' 导入类型 { DashboardOptions, TrendingProject, TechNewsItem, TemplateData };
从 './Lib/template-helpers' 导入 { registerHelpers, renderTemplate };
从'./Lib/visualization-helpers'导入{analyzeData}；
 ...
 * 构建 GitHub 趋势 URL
 */
函数 buildTrendingUrl（选项：DashboardOptions）：字符串 {
  const baseUrl = "https://github.com/trending";
  constsince=options.period===“每日”？ “每日”：“每周”；
  让 url = `${baseUrl}?since=${since}`;
...
异步函数 getTrendingProjects(options: DashboardOptions): Promise<TrendingProject[]> {
  ...
  const html = 等待响应.text();
  返回 parseTrendingProjects(html, options.limit);
}
  ...
  尝试{
    ...
    articlesToProcess.forEach((文章, 索引) => {
      尝试{
        ...
        // 提取stars总数 - GitHub 修改了 HTML 结构，数字在 SVG 后面
 ...
 * 获取技术新闻
 */
异步函数 getTechNews(count: number): Promise<TechNewsItem[]> {
  常量 HN_API = 'https://hn.algolia.com/api/v1/search_by_date';
...
<div class="bg-gradient-to-br from-green-500 to-emerald-600 rounded-xl p-6 text-whiteshadow-lg">
        <h3 class="text-lg font-semibold opacity-90">总星星数</h3>
        <p class="text-4xl font-bold mt-2">{{analytics.totalStars}}</p>
        <p class="text-sm opacity-75 mt-1">所有项目总计</p>
      </div>

      <div class="bg-gradient-to-br from-orange-500 to-red-500 rounded-xl p-6 text-whiteshadow-lg">
        <h3 class="text-lg font-semibold opacity-90">最热门项目</h3>
        <p class="text-xl font-bold mt-2 truncate">{{analytics.topProject.name}}</p>
        <p class="text-sm opacity-75 mt-1">{{analytics.topProject.stars}} 颗星</p>
      </div>
    </节>
      ...
      <div class="flex flex-wrap gap-4 items-center">
        <div class="flex-1 min-w-64">
          ...
          <输入
            ...
            class =“w-full px-4 py-2边框border-gray-300 rounded-lg焦点：ring-2焦点：ring-github-accent焦点：border-transparent”
        ...
        <div>
          ...
          <选择
            id="语言过滤器"
            class =“px-4 py-2边框border-gray-300 rounded-lg焦点：ring-2焦点：ring-github-accent焦点：border-transparent”
    ...
    <section class="bg-white rounded-xl Shadow-sm p-6 mb-8">
      ...
      <div class =“网格grid-cols-1 lg：grid-cols-2间隙-8”>
        <div>
...
<!-- 热门项目 -->
    <节类=“mb-8”>
      ...
      <div id="projects-container" class="grid grid-cols-1 gap-4">
        {{#每个项目}}
        <div class="project-card bg-white rounded-xlshadow-sm p-6 border border-gray-200"
             数据排名=“{{排名}}”
             数据语言=“{{语言}}”
             数据星=“{{星}}”
             数据增长=“{{starsThisPeriod}}”
             数据名称=“{{名称}}”
             数据描述 =“{{描述}}”>
          <div class="flex items-start justify- Between">
            <div class="flex-1">
              <div class="flex items-center gap-3 mb-2">
                ...
                <span class="badge bg-blue-100 text-blue-800">{{语言}}</span>
              </div>
              <p class="text-gray-600 mb-3">{{描述}}</p>
              <div class="flex items-center gap-4 text-sm text-gray-500">
                <span>⭐ {{stars}}颗星</span>
                {{#ifstarsThisPeriod}}
                  <span class="text-green-600 font-semibold">(+{{starsThisPeriod}} 这个 {{../period}})</span>
                {{/如果}}
              </div>
            </div>
            <a href="{{url}}" target="_blank" class="px-4 py-2 bg-github-accent text-white rounded-lg悬停：bg-blue-600过渡字体-medium">
              查看 →
            </a>
          </div>
        </div>
    ...
    <节类=“mb-8”>
      ...
      <div class="网格 grid-cols-1 间隙-4">
        {{#每条新闻}}
        <div class="news-item bg-white rounded-xlshadow-sm p-5 悬停：shadow-md 过渡">
          <div class="flex items-start justify- Between">
            <div class="flex-1">
  ...
  <footer class="bg-white border-t border-gray-200 mt-12">
    <div class="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
      <p class="text-center text-gray-500 text-sm">
        由 GitHubTrends 技能生成 | 数据来源：GitHub 和 Hacker News
      </p>
    </div>
  </页脚>
  ...
  <脚本>
    ...
    窗口.dashboardData = {
      ...
      分析：{
        ...
        增长数据：{{{json Analytics.growthData}}}
      }
    };
...
// 语言分布饼图
    ...
    函数 initGrowthChart() {
      ...
      const 项目 = window.dashboardData.projects.slice(0, 10);

      新图表（ctx，{
        类型：'酒吧'，
        数据：{
          标签：projects.map(p => p.name.split('/')[1] || p.name),
          数据集：[{
            标签：“这个时期的明星”，
            数据：projects.map(p => parseInt(p.starsThisPeriod.replace('+', '') || 0)),
            背景颜色: 'rgba(88, 166, 255, 0.8)',
    ...
    函数过滤器项目（）{
      ...
      const 卡 = document.querySelectorAll('.project-card');

      卡.forEach（卡=> {
        const name = card.dataset.name.toLowerCase();
        const 描述 = card.dataset.description.toLowerCase();
        const 语言 = 卡.数据集.语言;

        const matchesSearch = name.includes(searchValue) ||描述.includes(searchValue);
        const matchesLanguage = languageValue === '全部' ||语言 === 语言值；

        卡.样式.显示 = matchesSearch && matchesLanguage ? '块'：'无'；
      });
    }
    ...
    函数 sortProjects() {
      ...
      const 卡 = Array.from(container.children);

      卡片.sort((a, b) => {
        开关（排序依据）{
          案例“星星”：
            返回 parseInt(b.dataset.stars.replace(/,/g, '')) - parseInt(a.dataset.stars.replace(/,/g, ''));
          案例“增长”：
          ...
          案例“排名”：
          默认：
            返回 parseInt(a.dataset.rank) - parseInt(b.dataset.rank);
        }
      });

      卡.forEach(卡 => 容器.appendChild(卡));
    }
  </脚本>
</正文>
</html>
文件：工作流程/GenerateDashboard.md
# 生成仪表板工作流程
