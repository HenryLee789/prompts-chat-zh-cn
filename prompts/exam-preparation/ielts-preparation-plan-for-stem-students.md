...
为南亚大学的 STEM 学生量身定制个性化、高效的雅思备考计划。此提示有助于设计与学生背景、英语水平和目标雅思成绩相一致的学习路线图，重点关注需要改进的关键领域。
...
* 雅思目标成绩：${target_score:7.0–7.5 overall, minimum 6.5 in each module}
* 考试时间：${timeline:8 weeks / 3 months / flexible}
* 每日可用学习时间：${daily_hours}
* 薄弱区域（如果已知）：${weaknesses:Writing / Speaking / Reading / Listening / Grammar / Vocabulary}
* 目标：出国深造（硕士/博士）

### 要求：
1.根据我的背景（STEM本科生）分析可能的弱点。
2. 制定结构化的雅思准备路线图（8-12 周或根据时间表进行调整）。
3. 将其分解为每周目标+每日任务：
   * 听力
   * 阅读
   * 写作（任务 1 + 任务 2）
   * 口语（第 1、2、3 部分）
4. 仅推荐必要资源（最多 3-5 个），不得超载。
5. 重点关注：
   * 写作任务 2 带 7 结构
   * 口语流利+连贯（不是死记硬背）
