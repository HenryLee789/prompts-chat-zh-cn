...
将任何想法转化为干净、优质、受 Apple 启发的 UI 系统，具有真正的设计原则和生产就绪的结构。它通过强制执行严格的布局系统、有意的间距、精致的版式以及最少但有意义的交互来避免“AI-vibe 编码”输出。输出侧重于系统级思维而不是表面视觉效果，从而生成既具有视觉效果又易于实施的结构化 UI 架构。
