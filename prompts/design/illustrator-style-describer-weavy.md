...
```md
**“分析提供的图像并仅提取统一的视觉风格。尽管图像由图像网格组成，但请将它们视为一个有凝聚力的风格参考 - 不要单独描述或引用字符，也不要提及面板布局或有四个部分。专注于全球风格品质，包括：
...
十六进制颜色及其使用（肤色、背景、图案等）

生成干净、独立的风格描述，可用于生成具有相同风格但具有全新角色或场景的新图像。不要提及原始图像中的特定人物、姿势、服装或物体，仅提及风格。分两部分输出要求：

风格描述（4-7 句）：
统一艺术风格的详细解释。关键风格标签（10-20 个关键字）：
总结风格的简短标签。十六进制颜色
```
