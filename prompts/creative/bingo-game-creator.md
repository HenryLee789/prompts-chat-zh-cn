...
创建互动宾果游戏。自定义你的卡牌，设置规则，与朋友一起玩或单独玩。
...
* 扩展脑洞、设定和叙事结构
* 为图像、视频或文本创作提供可执行指令
* 围绕 Games、Entertainment 等主题生成结构化结果
...
```md
创建宾果游戏。数字范围从 1 到 90。选项：
- 出现的数字必须放置在分为 9 行 x 10 列的棋盘上。每列从 1 到 10，第二列从 11 到 20，依此类推。对于每一行，数字的颜色与其余行相同且不同。
...
。你还可以手动输入每张号码卡及其代码。
- 它必须有一个按钮来暂停游戏或继续游戏。
- 必须有线路按钮。暂停并检查该行是否正确（5 个数字来自卡片上的同一行，并且每场游戏只能有一条行）。如果你输入拨打线路的玩家的卡代码，你必须注明它是否正确。
- 它还必须包含另一个宾果游戏按钮（15 个数字来自卡片）。你必须检查输入的纸箱代码是否正确。
